...
ByteAddressBuffer         MeshletTriangleIndices  : register(t3);
...
{
    uint3 primitive;
    // I think the problem here is that these offsets are wrong
    uint xd = MeshletTriangleIndices.Load(m.PrimOffset + index * 4);
    primitive.z = (xd >> 0) & 0xFF;  // Least significant byte
    primitive.y = (xd >> 8) & 0xFF;  // Second byte
    primitive.x = (xd >> 16) & 0xFF; // Third byte
