...
HRESULT MeshletizedModel::LoadFromFile(const wchar_t* filename)
{
...
HRESULT MeshletizedModel::UploadGpuResources()
{
    ...
    for (uint32_t i = 0; i < m_meshes.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < m.Vertices.size(); ++j)
        {
            ...
            device->CreateCommittedResource(&defaultHeap, D3D12_HEAP_FLAG_NONE, &vertexDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m.VertexResources[j]));
...
HRESULT MeshletizedModel::set_CBV()
{
...
void MeshletizedModel::draw()
{
    set_CBV();
    auto cmd_list = Renderer::get_instance()->g_pd3dCommandList;
    cmd_list->SetGraphicsRootConstantBufferView(0, m_constant_buffer->GetGPUVirtualAddress() + sizeof(SceneConstantBuffer) * Renderer::get_instance()->frame_index);
    for (auto& mesh : m_meshes)
    {
        cmd_list->SetGraphicsRoot32BitConstant(1, mesh.IndexSize, 0);
        cmd_list->SetGraphicsRootShaderResourceView(2, mesh.VertexResources[0]->GetGPUVirtualAddress());
        cmd_list->SetGraphicsRootShaderResourceView(3, mesh.MeshletResource->GetGPUVirtualAddress());
        cmd_list->SetGraphicsRootShaderResourceView(4, mesh.UniqueVertexIndexResource->GetGPUVirtualAddress());
        cmd_list->SetGraphicsRootShaderResourceView(5, mesh.PrimitiveIndexResource->GetGPUVirtualAddress());
...
void MeshletizedModel::update()
{
    Drawable::update();
    draw();
}

void MeshletizedModel::create_CBV()
{
    int frameCount = 3;
