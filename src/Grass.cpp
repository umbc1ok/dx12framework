...
void Grass::draw_editor()
{
    ...
    ImGui::SliderFloat("Restoration strength factor", &wind->restoration_strength,0, 50);
    ...
    ImGui::SliderFloat("Pulsation delta per frame", &m_pulsationSpeed, 1, 5);
}
...
void Grass::set_constant_buffer()
{
    ...
    memcpy(m_cbv_wind_data_begin + sizeof(Wind) * Renderer::get_instance()->frame_index, wind, sizeof(m_constant_buffer_data));
    commandList->SetGraphicsRootConstantBufferView(1, m_wind_constant_buffer->GetGPUVirtualAddress() + sizeof(Wind) * Renderer::get_instance()->frame_index);
}
