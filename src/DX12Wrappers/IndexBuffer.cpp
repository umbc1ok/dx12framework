...
IndexBuffer::IndexBuffer(uint32_t const* data, uint32_t const no_of_indicies)
{
    m_data = data;
    ...
    Renderer::get_instance()->update_buffer_resource(cmdlist,
        ...
        no_of_indicies, sizeof(uint32_t), data);

    // Create index buffer view.
    m_IndexBufferView.BufferLocation = m_IndexBuffer->GetGPUVirtualAddress();
    m_IndexBufferView.Format = DXGI_FORMAT_R32_UINT;
    m_IndexBufferView.SizeInBytes = sizeof(uint32_t) * no_of_indicies;
